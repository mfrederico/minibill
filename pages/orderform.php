...
if (!$_REQUEST['uniq_id'])
{
	...
	if ($_GET['order_id'])
	{
		...
		$prodlist = $preOrder['item_id'].'-'.$preOrder['quantity'];
	...
	if ($res)
	{
		while($info = mysql_fetch_assoc($res))
		{
			...
			if ($info['id'] == $item_id) $info['price'] = $price;
...
else
{
	...
	while($info = mysql_fetch_assoc($res))
	{
		$Q="SELECT * FROM products WHERE id='$info[item_id]' LIMIT 1";
		$pres = mysql_query($Q);
		...
		if ($info['status'] != 'paid')
		{
			updateProd($item_id,'price',$info['amount']);
			updateProd($item_id,'title',$prod['title']);
			updateProd($item_id,'stock',$prod['stock']);
			$grand_total += ($info['price'] * $info['quantity']);
		}
	}
