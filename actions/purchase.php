...
if (isset($_POST['user']['id']) && isset($_POST['uniq_id']))
{
	$Q="SELECT email,password 
		FROM users,orders 
		WHERE id='".addslashes($_POST['user']['id'])."' 
		AND orders.user_id='".addslashes($_POST['user']['id'])."' 
		AND uniq_id='".addslashes($_POST['uniq_id'])."' LIMIT 1";
...
else
{
	...
	$X->assign('title',"Internal Error - my fat cat sat on a hat and spat on a bat");
	$thisPage = "error";
}
