...
if ($_REQUEST['user_id'])
	$Q="UPDATE users SET $set WHERE id='$_REQUEST[user_id]' LIMIT 1";
else 
	$Q="INSERT INTO users SET $set";

mysql_query($Q);

if (!$_REQUEST['user_id']) $_REQUEST['user_id'] = mysql_insert_id();
...
$redirect_to = ("index.php?page=editUser&id=$_REQUEST[user_id]&msg=$msg");
