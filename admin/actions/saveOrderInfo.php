...
{
	$Q = buildSet($_REQUEST['order'],'order_id','orders');
	mysql_query($Q);
	$msg = base64_encode("#00A000|#FFFFFF|Order Updated");
	$redirect_to = ("index.php?page=editOrder&order_id=".$_REQUEST['order']['order_id']."&msg=$msg");
}
//...... Delete this order (delete flag sent = $_REQUEST[delete])
else
{
	$ord = new OrderClass($user_id,0,$config);
	$order_count = $ord->delOrder($uniq_id,$_REQUEST['order']['order_id']);

	$msg = base64_encode("#00A000|#FFFFFF|Order ".$_REQUEST['order']['order_id']." Deleted");
