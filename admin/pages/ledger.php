...
switch ($_REQUEST[search][type])
{
	case "all":
		...
		GROUP BY 
			uniq_id ORDER BY last_billed DESC";
		break;
	case "name":
		$Q="SELECT $SELECT_FIELDS
		FROM 
			orders,users 
		WHERE 	
			(users.firstname LIKE '%".addslashes($phrase)."%' 
		OR 
			users.lastname LIKE '%".addslashes($phrase)."%')
		AND 
			orders.user_id=users.id
		GROUP BY 
			uniq_id ORDER BY lastname";
	...
	case "email" : 
		...
		FROM 
			orders,users 
		WHERE 
			users.email LIKE '%".addslashes($phrase)."%' 
		AND 
			orders.user_id=users.id
		GROUP BY 
			uniq_id 
		...
		break;
	case 'yesterday':
		$sdate = date('Y-m-d 00:00:00',time() - 86400);
		$edate = date('Y-m-d 23:59:59',time() - 86400);
		$Q="SELECT $SELECT_FIELDS
		FROM 
			orders,
			users 
